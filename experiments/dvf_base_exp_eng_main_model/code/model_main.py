...
class ModelMain(nn.Module):
    ...
    def forward(self, data, mode='train'):
        ...
        if mode in {'train', 'val'}:
            ...
            tgt_mask = Variable(subsequent_mask(self.opts.max_seq_len).type_as(ref_pad_mask.data)).unsqueeze(0).expand(z.size(0), -1, -1, -1).float()
            ...
            for t in range(0, trg_len):
                tgt_mask = Variable(subsequent_mask(sampled_svg.size(0)).type_as(ref_seq_cat.data)).unsqueeze(0).expand(sampled_svg.size(1), -1, -1, -1).float()
    ...
    def fetch_data(self, data, mode):
        ...
        if mode == 'train':
            ...
            ref_cls = torch.arange(0, self.opts.ref_nshot, 1).unsqueeze(0).expand(input_image.size(0), -1)
        else:
            ref_ids = self.opts.ref_char_ids.split(',')
            ref_ids = list(map(int, ref_ids))
            assert len(ref_ids) == self.opts.ref_nshot
            ref_cls = torch.tensor(ref_ids).unsqueeze(0).expand(self.opts.char_num, -1)
        ...
        ref_pad_mask = ref_pad_mask.float().unsqueeze(1)
